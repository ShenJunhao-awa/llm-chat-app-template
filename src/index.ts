...
注意：用户提交的内容中 **可能包含标题+正文**，你需要检查**全部内容**（标题和正文都要看）。
...
【输出格式 - 重要】
- 必须检查全文，**有几处违规就列几条**，不要合并，不要遗漏
- 安全：{"status":"pass"}
- 违规：{"status":"reject", "violations":[{"reason":"2-6字概括", "violation_phrase":"原样抄写违规词句"}, {"reason":"2-6字概括", "violation_phrase":"原样抄写违规词句"}]}
- 每条违规单独一条 violations，reason 由AI自己总结2-6字
- 只返回JSON`;
